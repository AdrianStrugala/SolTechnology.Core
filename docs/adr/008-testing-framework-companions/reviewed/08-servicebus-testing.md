...
- **Lifetime is the headline.** Carry over verbatim from KYC:
  ...
  - **Reuse via stable container name + manual Docker.DotNet management** (not Testcontainers' reuse hash, which is unstable because of the `MsSqlContainer` reference in `DependsOn`): on first run create via Testcontainers with a fixed name; on subsequent runs detect by name, start-if-stopped, re-read mapped port, rebuild connection string.
  - **Semaphore-guarded, per-instance one-time init** + `ConcurrentDictionary` caches for connection strings and initialized flags.
