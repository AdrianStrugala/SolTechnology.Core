---
name: command-query-event-story
description: Author a use case in any app built on the `SolTechnology.Core.CQRS` / `SolTechnology.Core.Story` NuGet packages — a command, a query, a fire-and-forget event, or a Story (chapters). Covers simple handlers, Stories hosted in `Commands`/`Queries`, domain-model Stories in `DomainServices`, and long-running interactive Stories in `Workflows`. Use whenever adding a new command/query/event, promoting a handler past the §3 size budget into a Story, adding a chapter, publishing or handling an `IEvent`, wiring a handler into a `ModuleInstaller`, or adding `[LogScope]` logging. Encodes the layout and rules from `ClaudeCodingGuide.md` §0/§3/§4/§11 and the DreamTravel reference app. Routes tests to `test-writing`, review to `code-review`, behaviour-preserving cleanup to `refactor`.
---

# Command-Query-Event-Story

Procedure for writing a single use case the Tale Code way — in the DreamTravel sample **or in any
app that consumes the `SolTechnology.Core.CQRS` / `SolTechnology.Core.Story` NuGet packages**. Where
...
The CQRS/Story **contracts** (`IQueryHandler<,>`, `ICommandHandler<>`, `IEvent`, `StoryHandler<,,>`,
...
- **Always** when adding a new command, query, or event — in a sample app (`DreamTravel`,
  `TaleCode`) or in a consumer app built on the `SolTechnology.Core.CQRS` / `.Story` packages.
- A handler grew past **~100 lines of business logic** or now talks to **> 1 external system** —
  §3 requires promotion to a Story (chapters).
- Adding a chapter to an existing Story, or adding an `InteractiveChapter` pause point.
- Publishing a domain event (`IMediator.Publish`) or writing its `IEventHandler<>`.
- Deciding where a Story lives: dedicated to a `Queries`/`Commands` use case vs a write-side
  `DomainServices` Story vs a long-running interactive `Workflows` Story.

## When NOT to use

- Adding or changing a **public/protected symbol** in `src/SolTechnology.Core.*` (the CQRS/Story
...
- [`ClaudeCodingGuide.md` §4](../../../docs/ClaudeCodingGuide.md) — Story anatomy, `TellStory()` no-logic rule, chapters, `DomainServices` vs `Workflows`.
...
- [`CQRS.md`](../../../docs/CQRS.md) / [`Story.md`](../../../docs/Story.md) — public API of the two libraries.
- [`Hangfire.md`](../../../docs/Hangfire.md) — durable (persisted) event dispatch.
- DreamTravel reference implementations:
  - Query-as-Story — `sample-tale-code-apps/DreamTravel/src/LogicLayer/DreamTravel.Queries/CalculateBestPath/`
  ...
  - Story in DomainServices — `.../DreamTravel.DomainServices/CityDomain/SaveCityStory/`
...
- **Handlers never throw for business failures.** Return `Result` / `Result<T>`. Throwing is
  reserved for true exceptions (network, IO) — the Story engine converts those to errors (§4).
- **`TellStory()` is a table of contents.** No `if`, no `try`, no loops, no logic — only
...
| Question | Artifact | Interface (from `SolTechnology.Core.CQRS` / `.Story`) | Return | DreamTravel example |
|---|---|---|---|---|
...
| ≥ 2 steps / > 1 external system / > ~100 lines? | **Story** | `StoryHandler<TInput, TContext, TOutput>` (+ optionally `IQueryHandler<,>` / `ICommandHandler<,>`) | `Result<TOutput>` | `CalculateBestPath` |
| Something happened, others may react? | **Event** | `IEvent` + `IEventHandler<TEvent>` | `void` publish / `Task` handle | `CitySearched` |

Hosting a Story (§4):

- **`Queries/`** — a complex query Story is **always dedicated to its use case**. Never extract a
  query into a domain service.
- **`Commands/`** — a single command triggered by one entry point. Default for writes.
- **`DomainServices/`** — the Story works **directly on domain models** (save / update / mutate) and
  is **reused by multiple commands or event handlers** (e.g. `SaveCityStory` behind
  `ICityDomainService.Save`, reused by the `CitySearched` event handler). Write-side only — expose a
  plain interface, inherit `StoryHandler` internally, and keep chapters under the service folder.
- **`Workflows/`** — long-running, **interactive** (`InteractiveChapter`), **persisted**. Requires
  `RegisterStories(StoryOptions.WithSqlitePersistence(...))`.
...
<UseCase>/
  ...
  <UseCase>Story.cs        ← StoryHandler implementation
  Chapters/
    0.<FirstVerb>.cs
    1.<SecondVerb>.cs
```
...
### 5. Author the handler — or decide it is a Story
...
stop and convert to a Story (step 6) — this is a §3 hard rule, not a preference.

### 6. Author the Story

- `Context` inherits `Context<TInput, TResult>` and carries every cross-chapter field.
- The `StoryHandler<TInput, TContext, TResult>` passes `(serviceProvider, logger)` to the base
  ctor and may also implement `IQueryHandler<,>` / `ICommandHandler<,>` so the mediator resolves
  it directly (see `CalculateBestPathStory`).
- `TellStory()` lists chapters top-to-bottom — prose only, zero logic.
...
token (see `CustomerDetailsChapter`). The workflow needs SQLite persistence registered (step 10)
— a `WithoutPersistence()` engine fails an interactive chapter with a clear error (Story.md).
...
- Each `IEventHandler<TEvent>.Handle(...)` runs in its own DI scope. Place handlers in the
  consumer (`Worker`/`Presentation`), one folder per event (`OnCitySearched/`).
...
`AddCQRS`) emits a **START** on entry and a **SUCCESS** / **FAIL** with duration on exit, keyed by
...
- **Event handlers and chapters are NOT on the pipeline.** Events dispatch through the
  fire-and-forget publisher and chapters run inside the Story engine, so neither gets the automatic
...
- CQRS module: `services.AddCQRS(assemblies: typeof(ModuleInstaller).Assembly);`
- Story module: `services.RegisterStories(assemblies: typeof(SomeStory).Assembly);`
- Persisted workflow: `services.RegisterStories(StoryOptions.WithSqlitePersistence("…"), …)`.
...
- [ ] Artifact classified (command / query / event / story) and placed in the correct layer (§0)
      and host project (§4).
- [ ] One folder per use case; files named `<UseCase>{Query|Command|Result|Context|Story}.cs`.
...
- [ ] Handler ≤ ~100 lines of business logic **and** touches ≤ 1 external system — else it is a Story.
- [ ] `TellStory()` contains only `ReadChapter<…>()` calls plus output mapping — no `if`/`try`/loop.
...
- [ ] Registration is assembly-scan (`AddCQRS` / `RegisterStories`); only true collaborators added explicitly.
...
- DO NOT put logic, branching, or loops in `TellStory()`. Split into chapters instead.
...
- DO NOT add or change a public/protected symbol in `src/SolTechnology.Core.*` here. That is the
  framework surface — route to `implementation-planning` + `premortem`.
- DO NOT exceed the §3 promotion threshold without converting to a Story.
...
- DO NOT improvise a freehand use-case layout when this skill is unavailable. STOP and tell the
  user `command-query-event-story` is required (`CLAUDE.md §2`). Freehand handlers are how
  100-line god-handlers and logic-in-`TellStory()` re-enter the codebase.
