...
namespace SolTechnology.Core.MessageBus.Broker
{
    public class MessageBusBroker : IMessageBusBroker, IAsyncDisposable
    {
        ...
        public void RegisterTopicPublisher(string messageType, string topicName)
        {
            ...
            if (_createResources && _managementClient != null)
            {
                if (!_managementClient.TopicExistsAsync(topicName).GetAwaiter().GetResult().Value)
                {
        ...
        public void RegisterTopicReceiver(Type messageType, string topicName, string subscriptionName)
        {
            ...
            if (_createResources && _managementClient != null)
            {
                if (!_managementClient.SubscriptionExistsAsync(topicName, subscriptionName).GetAwaiter().GetResult().Value)
                {
        ...
        public void RegisterQueuePublisher(string messageType, string queueName)
        {
            ...
            if (_createResources && _managementClient != null)
            {
                if (!_managementClient.QueueExistsAsync(queueName).GetAwaiter().GetResult().Value)
                {
